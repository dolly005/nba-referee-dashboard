...
def load_daily_data(
    ...
    with get_connection() as conn:
        ...
        try:
            games_df = transformed.get("games", pd.DataFrame())
            game_officials_df = transformed.get("game_officials", pd.DataFrame())

            if games_df.empty:
                raise ValueError(
                    "No games were extracted for this date. "
                    "This may be caused by NBA Stats API timeout, request blocking, "
                    "or a date with no available games."
                )
